...
public class MyMybatisGeneratorService extends MapperGeneratorStrategyBase {
    ...
    public void generator(GeneratorConfig generatorConfig) {
        ...
        context.setAttribute("targetRuntime", mybatisExportConfig.getTargetName());
        ...
        // 是否成成注释
        if (mybatisExportConfig.isUseComment()) {
        ...
        // 默认类型解析器
        if (mybatisExportConfig.isUserJava8() && mybatisExportConfig.isUseBigDecimal()) {
        ...
        } else if (mybatisExportConfig.isUserJava8()) {
            final Element javaTypeResolver = this.addElement(context, "javaTypeResolver");

            final Element useJSR310TypesProperty = this.addElement(javaTypeResolver, PROPERTY);
            useJSR310TypesProperty.setAttribute(NAME, "useJSR310Types");
            useJSR310TypesProperty.setAttribute(VALUE, "true");
        } else if (mybatisExportConfig.isUseBigDecimal()) {
            final Element javaTypeResolver = this.addElement(context, "javaTypeResolver");
